...
number_tree = 
  ...
  mutate(borough.x = fct_reorder(borough.x, n_trees))

ggplot(data = number_tree, aes(x = borough.x, y = n_trees, fill = borough.x)) + geom_bar(stat = 'identity') + geom_text(aes(label = n_trees), size = 3, vjust = -1) + 
  labs(
...
mhtn_number_tree = 
  ...
  pivot_longer(
   ...
   mutate(nta_name.x = fct_reorder(nta_name.x, total))
  

ggplot(mhtn_number_tree, aes(x = nta_name.x, y = total, fill = type)) + geom_bar(stat = 'identity', position = 'dodge')+ theme(axis.text.x = element_text(angle = 65, hjust=1)) + 
  labs(
