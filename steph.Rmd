...
## Asthma Emergency Dept Age 4 and younger 2014-2016
### load Asthma Emergency Dept Age 4 and younger 2014-2016
```{r}
asthma_ed_child_raw <-
  read_csv("small_data/NYC EH Data Portal_Asthma emergency department visits_4 and under.csv") |> 
  janitor::clean_names()

asthma_ed_child_clean <- asthma_ed_child_raw |> 
  rename(nta_name = geography) |> 
  filter(geo_type == "NTA2010" & time == "2014-2016") |> 
  select(nta_name, average_annual_rate_per_10_000, number)


```

### Summarize trees per acre by nta and asthma emergency dept child
checked and no NAs are present
```{r}
trees_and_asthma_ed_child <- left_join(trees_per_acre_df, asthma_ed_child_clean, by = "nta_name")

sum(is.na(trees_and_asthma_ed_child))

```

### Tables and Graphs of trees per acre and asthma ED child and num trees WORKING
...
trees_and_asthma_ed_child |> 
  ...
  arrange(desc(trees_per_acre)) |> 
  filter(min_rank(desc(trees_per_acre)) < 11) |> 
  knitr::kable(digits = 2, caption = "Top 10 neighborhoods with most trees per acre and their asthma ED child rate")

trees_and_asthma_ed_child |> 
  select(nta_name, num_trees, total_acres, trees_per_acre, average_annual_rate_per_10_000) |> 
  arrange((trees_per_acre)) |> 
  filter(min_rank(trees_per_acre) < 11) |> 
  knitr::kable(digits = 2, caption = "Top 10 neighborhoods with least trees per acre and their asthma ED child rate")
